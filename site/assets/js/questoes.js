...
async function lerPDF() {
    ...
    if (lista && lista.children.length > 0) if (!confirm("Substituir lista atual?")) return;
    let inp = el("imp-file"); if (inp.files.length === 0) return alert("Selecione PDF");
    ...
    } catch (e) { alert("Erro ao ler PDF."); } finally { hideLoader(); }
}
...
function renderPreview(lista) {
    ...
    lista.forEach((q, i) => {
        let statusHtml = q.ja_cadastrada ? `<span class="aviso-dup">⚠️ Já Cadastrada</span>` : "";
...
<div class="${classeRow}" id="imp-row-${i}">
    ...
    <div class="imp-meta">
        ...
        <input type="text" class="imp-assunto-ind" placeholder="Assunto" value="${q.assunto}" list="lista-assuntos" style="font-size:0.85em; color:var(--purple)">
...
function verificarDuplicidadeDinamica(index) {
    ...
    checkDupTimeout = setTimeout(async () => {
        ...
        let payload = { enunciado: row.querySelector(".imp-enunciado").value, alt_a: row.querySelector(".imp-alt-a").value };
        if (!payload.enunciado) return;
        const resp = await fetch(`${API}/check-duplicidade`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
        const data = await resp.json();
        el(`status-${index}`).innerHTML = data.existe ? `<span class="aviso-dup">⚠️ Já Cadastrada</span>` : "";
...
async function salvarIndividual(index) {
    ...
    let assuntoFinal = r.querySelector(".imp-assunto-ind").value || el("imp-assunto").value || "Geral";
    ...
    formData.append("alt_a", r.querySelector(".imp-alt-a").value); formData.append("alt_b", r.querySelector(".imp-alt-b").value);
    formData.append("alt_c", r.querySelector(".imp-alt-c").value); formData.append("alt_d", r.querySelector(".imp-alt-d").value);
    ...
    if (fileInput && fileInput.files[0]) formData.append("imagem_file", fileInput.files[0]);
    if (!formData.get("enunciado") || !formData.get("gabarito")) return alert("Dados incompletos.");
    try {
        const response = await fetch(`${API}/questoes`, { method: "POST", body: formData });
        if (response.ok) {
            r.classList.add("sucesso-salvo");
            r.querySelector(".imp-acoes").innerHTML = `<span style="color:green; font-weight:bold">Salva!</span>`;
            init();
        } else if (response.status === 409) { alert("Duplicada!"); }
...
async function salvarLote() {
    ...
    let lote = [];
    rows.forEach(r => {
        if (r.classList.contains("sucesso-salvo") || r.classList.contains("ja-cadastrada") || r.querySelector(".imp-imagem-file").files.length > 0) return;
    ...
    let ok = 0;
    for (let q of lote) {
        try { if ((await fetch(`${API}/questoes`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(q) })).ok) ok++; } catch (e) { }
    }
    hideLoader(); alert(`${ok} Salvas!`); init();
}
...
function filtrar() {
    ...
    let tb = document.querySelector("#tabela-questoes tbody"); tb.innerHTML = "";
    filtrados.forEach(q => {
    ...
    <td>${q.gabarito}</td>
    <td><button class="btn-icon" onclick="abrirEd('${q.id}')">✏️</button><button class="btn-icon" onclick="del('${q.id}')">🗑️</button></td>
</tr>`;
    });
}
...
function abrirEd(id) {
    ...
    el("edit-id").value = q.id; el("edit-enunciado").value = q.enunciado; el("edit-instituicao").value = q.instituicao || ""; el("edit-ano").value = q.ano || ""; el("edit-dificuldade").value = q.dificuldade; el("edit-tipo").value = q.tipo; el("edit-banca").value = q.banca; el("edit-disciplina").value = q.disciplina; carregarAssuntos('edit'); el("edit-assunto").value = q.assunto; altTipo("edit");
    ...
    el("edit-imagem-nome").value = q.imagem || ""; el("edit-imagem-file").value = "";
    el("edit-img-preview-container").innerHTML = q.imagem ? `<a href="${API}/img/q_img/${q.imagem}" target="_blank"><img src="${API}/img/q_img/${q.imagem}" class="img-preview-mini"></a>` : "<span style='font-size:0.8em;color:#999'>Sem imagem</span>";
